...
for j = 2:n-1
    for k = 2:m-1
        s(j,k) = i(j-1,k-1)+i(j-1,k)+i(j-1,k+1)+i(j,k-1)+i(j,k+1)+i(j+1,k-1)+i(j+1,k)+i(j+1,k+1)-8*i(j,k);
        lap(j,k) = (1/3)*abs(s(j,k));
        if(lap(j,k))>=25
            contour(j,k)=255;
        else 
            contour(j,k)=0;
...
figure; imshow(contour);impixelinfo; title('Контуры "Метод оператора Лапласа 19');